...
public class GameServiceMockTest {
    ...
    public void shouldMakeDraw() {
        ...
        Mockito.when(moveValidatorService.validate(Mockito.anyList(), Mockito.anyLong(), Mockito.anyLong(),
                Mockito.anyString(), Mockito.anyString()))
                .thenReturn(ValidationResult.validResult());

        //when
        BoardStatusEnum boardStatus = gameService
                .makeMove(1000L, 1L, 1L, "FakeName", "O")
                .getBoardStatus();
        ...
        Assert.assertEquals(Arrays.asList(1000L, 1L, 1L), captorForLong.getAllValues());
